...
## 2026-07-24 - Added aria-pressed to custom button groups
**Learning:** Custom button groups that function as radio buttons or single-select options (like issue type selectors) need `aria-pressed` to communicate their selected state to screen readers, since visual cues (like background color changes) are not announced.
**Action:** Always map the active selection state to `aria-pressed` on custom button components that act as selectors.
