...
const SURFACES = [
  ...
  ["admin_ops", ["src/app/admin", "src/app/api/admin"], "high", true, ["npm run check:ui:coverage", "npm run check:admin-browser-surface-smoke", "npm run test:contracts", "npm run check:continuity"], ["runtime_health_admin_debug_observability", "route_boundary_auth_request_guard"], ["admin_ui_chart_health", "route_runtime_health", "server_diagnostics", "admin_panel_system_logs"], [], ["Operational surfaces require source-first admin UI coverage, route contracts, selectors, and evidence-boundary checks by default; authenticated browser smoke is optional diagnostic reproduction and does not clear admin truth samples."]],
...
function rulesFor(commandName: string) {
  ...
  if (commandName === "check:ui:audits") return { requiredWhen: ["Touching public/user-facing UI surfaces when source coverage promotes browser reproduction."], optionalWhen: ["Admin UI defaults to source coverage and check:admin-browser-surface-smoke; pure backend or governance-only changes."], cleanupExpected: ["Produces .next and Playwright artifacts."], broadSignoffWeight: 6, verifies_runtime: false, verifies_ui: true, verifies_rules: false, verifies_inventory: false, verifies_continuity: false };
  ...
  if (commandName === "check:admin-browser-surface-smoke:browser") return { requiredWhen: ["Only when a source-reported admin UI issue needs browser reproduction or an explicit runtime visual contract promotes it."], optionalWhen: ["Default admin readiness uses source checks, route contracts, selectors, hydration markers, and fixture-backed reports."], cleanupExpected: ["May produce Playwright report/test-results and optional evidence fragments."], broadSignoffWeight: 3, verifies_runtime: false, verifies_ui: true, verifies_rules: false, verifies_inventory: false, verifies_continuity: false };
  if (commandName === "check:ui:lighthouse") return { requiredWhen: ["Only when source coverage explicitly promotes performance reproduction for loading/rendering behavior or a formal runtime performance contract requires it."], optionalWhen: ["Default UI continuity uses source coverage and runtime source checks."], cleanupExpected: ["Produces build and lighthouse output."], broadSignoffWeight: 3, verifies_runtime: false, verifies_ui: true, verifies_rules: false, verifies_inventory: false, verifies_continuity: false };
  ...
  if (commandName === "check:ui:continuity") return { requiredWhen: ["Broad UI hardening, UI coverage registry, or source-level UI continuity checks."], optionalWhen: ["Browser and Lighthouse diagnostics stay in check:ui:audits and check:ui:lighthouse when explicitly promoted; runtime/not-found checks stay in check:ui:runtime."], cleanupExpected: [], broadSignoffWeight: 7, verifies_runtime: false, verifies_ui: true, verifies_rules: false, verifies_inventory: true, verifies_continuity: true };
