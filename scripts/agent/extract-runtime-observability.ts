...
export function buildRuntimeObservability() {
  const lanes: RuntimeObservabilityLane[] = [
    ...
    {
      stable_id: toStableId("observability", "viewer_watch_capture_health"),
      key: "viewer_watch_capture_health",
      storage_or_collection: "analytics_watch_sessions",
      purpose: "Canonical watch-session capture quality lane for replay recovery, flush degradation, close-path misses, and viewer session continuity health.",
      intended_use: "Detect watch-time/session analytics degradation before historical dashboards or downstream rollups become misleading.",
      non_goals: [
        "Replacing the canonical watch-session documents themselves.",
        "Serving as a client-facing analytics chart.",
      ],
      persisted: true,
      raw_or_rollup: "status_items",
      client_or_server_origin: "mixed",
      trust_notes: ["Canonical watch-session documents remain the stronger truth; this lane summarizes continuity and capture quality from those documents."],
      backingPaths: [
    ...
    {
      stable_id: toStableId("observability", "analytics_export_status"),
      key: "analytics_export_status",
      storage_or_collection: "analytics_export_status",
      purpose: "Exporter heartbeat lane for warehouse delivery of first-party analytics facts into BigQuery.",
      intended_use: "Expose BigQuery raw-event export success/failure in Admin Debug and continuity checks.",
      non_goals: [
        "Replacing BigQuery table-level validation.",
        "Serving as the canonical analytics event source.",
      ],
      persisted: true,
      raw_or_rollup: "status_items",
      client_or_server_origin: "server",
      trust_notes: ["Firestore analytics_event_facts remains the canonical first-party source; this lane proves or disproves downstream warehouse delivery."],
      backingPaths: [
