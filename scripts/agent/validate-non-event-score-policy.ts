  ...
  return status.split(/\r?\n/u).filter(Boolean).map((line) => {
    ...
    const classification = path === STATE_PATH || path === SCORE_PATH || path === "agent/state/current-beta-exit-status.generated.json"
      ...
      : path === "agent/state/event-liveness-audit.generated.json"
        ? "current_generated_artifact_to_commit"
        : path.startsWith("agent/state/") && path.endsWith(".generated.json")
          ? "current_generated_artifact_to_commit"
      : path === DOC_PATH || path === "docs/agent-truth/final-testing-tracking-telemetry-lock.md" || path === "docs/agent-truth/current-beta-exit-status.md"
        || path === "docs/agent-truth/debug-signal-actionability.md"
        || path === "docs/agent-truth/debug-signal-grouping.md"
        || path === "docs/agent-truth/future-activity-signal-reclassification.md"
        ? "documentation_artifact_expected"
        : path === "docs/agent-truth/event-liveness-audit.md"
          ? "documentation_artifact_expected"
          : path.startsWith("docs/agent-truth/")
            ? "documentation_artifact_expected"
        : path === "CHANGELOG.md" || path === "public/kandydrops-release-notes.json" || path.startsWith("src/lib/release-notes/")
          ...
          : path === "package.json"
            ...
            : path === "src/lib/analytics/event-liveness-contract.ts" || path === "src/lib/analytics/event-liveness-engine.ts" || path === "src/lib/debug/debug-panel-tracking-summary.ts" || path === "src/lib/server/admin-debug/summary.ts" || path === "src/app/api/admin/debug/route.ts"
              ? "score_policy_source_change"
              : path === "src/lib/analytics/event-translation-bridge.ts" || path === "src/lib/analytics/person-metrics-hydration.ts" || path === "src/lib/testing/telemetry-trigger-test-matrix.ts"
                ? "score_policy_source_change"
              : path === "scripts/agent/validate-event-liveness-audit.ts"
                ? "score_validator_source_change"
                : path === "scripts/agent/validate-debug-signal-actionability.ts" || path === "scripts/agent/validate-debug-signal-grouping.ts" || path === "scripts/agent/validate-future-activity-signal-reclassification.ts"
                  ? "score_validator_source_change"
