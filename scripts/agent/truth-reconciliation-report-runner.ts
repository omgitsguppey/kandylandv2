...
export function runTruthReconciliationReport(kind: TruthReconciliationReportKind) {
  ...
  const outputReport = compactTruthReconciliationReport(kind, report);
  ...
  writeFileSync(join(ROOT, output.statePath), `${JSON.stringify(outputReport, null, 2)}\n`);
  writeFileSync(join(ROOT, output.docPath), renderTruthReconciliationDoc(kind, outputReport));
