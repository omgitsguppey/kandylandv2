...
function CoverPreviewCta({ truth, unlockCost, authLoading, unlocking, confirming, onCtaClick, onShare }: Pick<LockedDropPreviewViewProps, "truth" | "authLoading" | "unlocking" | "confirming" | "onCtaClick" | "onShare"> & { unlockCost: number }) {
    ...
    return (
        <button type="button" aria-busy={authLoading || unlocking} onClick={action} disabled={authLoading || unlocking} className={cn("mx-auto flex min-h-11 w-full max-w-[15rem] items-center justify-center gap-2 rounded-[0.95rem] border border-brand-purple/70 bg-black/70 px-3 py-2 text-xs font-black text-white shadow-[0_0_22px_rgba(164,118,255,0.24)] backdrop-blur-xl active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60", truth.shouldShowUnwrapCta && "bg-gradient-to-r from-brand-purple to-purple-500", truth.shouldShowCreatorShareCta && "border-white/20 bg-white/12")}>
