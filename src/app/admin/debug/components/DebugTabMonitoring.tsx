                        ...
                        <div className="divide-y divide-white/10">
                            {(data?.taskRollups || []).map((rollup: any) => (
                                <div key={rollup.taskId} className="space-y-2 px-4 py-3">
                                    ...
                                    <div className="flex flex-wrap gap-2"><Pill label="Assigned" value={rollup.assigned || 0} truthState="live" badgeLabel="LOADED" /><Pill label="Started" value={rollup.started} truthState="live" badgeLabel="LOADED" /><Pill label="Failed" value={rollup.failed} tone={rollup.failed ? "warn" : "good"} truthState="live" badgeLabel="LOADED" /><Pill label="Reminders" value={rollup.reminders} truthState="live" badgeLabel="LOADED" /><Pill label="Paid rewards" value={`${rollup.paidRewardTotalGd || 0} GD`} truthState="live" badgeLabel="LOADED" /><Pill label="Potential assigned" value={`${rollup.potentialRewardTotalGd || 0} GD`} truthState="live" badgeLabel="LOADED" /><Pill label="Forfeited potential" value={`${rollup.forfeitedPotentialRewardGd || 0} GD`} truthState="live" badgeLabel="LOADED" /><Pill label="Out of bounds" value={rollup.outOfBoundsEventCount || 0} tone={(rollup.outOfBoundsEventCount || 0) > 0 ? "warn" : "good"} truthState="live" badgeLabel="AUDIT" /></div>
                                </div>
                            ))}
                            {(data?.dailyTaskSeries || []).map((day: any) => (
                                <div key={day.dayKey} className="flex flex-wrap items-center gap-2 px-4 py-3 text-sm text-gray-300">
                                    ...
                                    <Pill label="Events" value={day.eventCount} truthState="live" badgeLabel="LOADED" /><Pill label="Assigned" value={day.assigned || 0} truthState="live" badgeLabel="LOADED" /><Pill label="Completed" value={day.completed} truthState="live" badgeLabel="LOADED" /><Pill label="Failed" value={day.failed} tone={day.failed ? "warn" : "good"} truthState="live" badgeLabel="LOADED" /><Pill label="Paid rewards" value={`${day.paidRewardTotalGd || 0} GD`} truthState="live" badgeLabel="LOADED" /><Pill label="Potential assigned" value={`${day.potentialRewardTotalGd || 0} GD`} truthState="live" badgeLabel="LOADED" /><Pill label="Forfeited potential" value={`${day.forfeitedPotentialRewardGd || 0} GD`} truthState="live" badgeLabel="LOADED" /><Pill label="Out of bounds" value={day.outOfBoundsEventCount || 0} tone={(day.outOfBoundsEventCount || 0) > 0 ? "warn" : "good"} truthState="live" badgeLabel="AUDIT" />
                                </div>
                            ))}
                        </div>
                    </ScrollWrap>
