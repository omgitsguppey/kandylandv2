    ...
    return (
        <div className="space-y-4 md:space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-1">
                <StatCard label="System Truth" value={data?.opsHealth?.canonicalState?.status === "healthy" ? "live" : data?.opsHealth?.canonicalState?.status || "--"} meta={data?.opsHealth?.canonicalState?.reason || "Awaiting canonical state"} truthState={!data && isLoading ? "loading" : error ? "failed" : data?.opsHealth?.canonicalState?.status === "healthy" ? "live" : "degraded"} />
