                        ...
                        <div className="p-4 flex flex-col gap-2">
                            {config.queue.length === 0 ? (
                                ...
                                config.queue.map((dropId, index) => {
                                    ...
                                    return (
                                        <div key={`${dropId}-${index}`} className="flex items-center gap-4 p-3 rounded-xl bg-black/40 border border-white/5 hover:bg-white/5 transition-colors group">
                                            <div className="text-gray-500 cursor-grab active:cursor-grabbing opacity-50 group-hover:opacity-100">
                                                <GripVertical className="w-5 h-5" />
                                            </div>

                                            <div className="w-12 h-12 rounded-lg bg-zinc-800 border border-white/10 overflow-hidden relative shrink-0">
                                                {drop.imageUrl ? (
                                                    <NextImage src={drop.imageUrl} alt={drop.title} fill sizes="48px" className="object-cover" />
                                                ) : null}
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-xs bg-brand-purple/20 text-brand-purple px-2 py-0.5 rounded border border-brand-purple/20 font-mono">
                                                        Slot {index + 1}
                                                    </span>
                                                    {sim && (
                                                        <span className="text-xs text-gray-400 flex items-center gap-1">
                                                            <Calendar className="w-3 h-3" /> Est: Day {Math.floor(index / config.dropsPerDay) + 1} @ {sim.timeStr}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-1 shrink-0">
                                                <div className="flex flex-col gap-1">
                                                    <button
                                                        ...
                                                        disabled={index === 0}
                                                        className="p-1 rounded bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 disabled:opacity-30 transition-colors"
                                                    >
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 ml-2 rounded-lg bg-red-500/10 text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-colors"
