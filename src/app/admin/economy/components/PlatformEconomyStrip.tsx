...
export function PlatformEconomyStrip({
    ...
    return (
        <div className="sticky top-[72px] z-10 grid gap-2 rounded-[1.2rem] border border-white/10 bg-zinc-950/95 p-2 backdrop-blur md:grid-cols-7">
            {STRIP_ITEMS.map((item) => {
                ...
                return (
                    <div key={item.key} className="rounded-xl border border-white/8 bg-white/[0.04] px-3 py-2">
                        ...
                        <div className="mt-1 text-sm font-semibold text-white">{formatGd(value ?? null)}</div>
                    </div>
                );
            })}
            <div className="rounded-xl border border-white/8 bg-white/[0.04] px-3 py-2">
                ...
                <div className="mt-1 text-sm font-semibold text-white">{formatUsdRate(treasury?.paidSourceAvgUsdPer100Gd ?? null)}</div>
            </div>
            <div className="rounded-xl border border-white/8 bg-white/[0.04] px-3 py-2">
                <div className="text-[11px] uppercase tracking-[0.16em] text-gray-500">Floor state</div>
                <div className="mt-1 text-sm font-semibold text-white">{treasury?.floorState ?? "unknown"}</div>
            </div>
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 px-3 py-2">
                ...
                <div className="mt-1 text-sm font-semibold text-white">{warningCount.toLocaleString()}</div>
            </div>
        </div>
    );
}
