...
export default function AdminAnalyticsPage() {
  ...
  const liveBlockingIssues = !liveResponse && liveError
    ? [liveError.message || "Realtime analytics request failed."]
    : [];
  const historicalBlockingIssues = !historicalResponse && historicalError
    ? [historicalError.message || "Historical analytics request failed."]
    : [];
  const liveBackgroundIssues = [
    liveResponse && liveError ? `Realtime analytics refresh failed: ${liveError.message || "Background refresh failed."}` : null,
    ...(liveResponse?.issues || []).map((issue) => `Realtime analytics issue: ${issue}`),
  ].filter((issue): issue is string => Boolean(issue));
  const historicalBackgroundIssues = [
    historicalResponse && historicalError ? `Historical analytics refresh failed: ${historicalError.message || "Background refresh failed."}` : null,
    ...(historicalResponse?.issues || []).map((issue) => `Historical analytics issue: ${issue}`),
  ].filter((issue): issue is string => Boolean(issue));
  ...
  const analyticsSectionHealth = [
      buildAdminUiChartHealthItem({
        ...
        hasData: liveSeries.length > 0 || (liveResponse?.totalActive ?? 0) > 0 || (liveResponse?.deepTrackerActive ?? 0) > 0,
        ...
        emptySummary: "Live pulse loaded without any realtime or recent historical signal.",
      }),
      buildAdminUiChartHealthItem({
  ...
  return (
    <div className="space-y-4 pb-20 md:space-y-5 md:pb-8">
      ...
      <div className="sticky top-[8.6rem] z-20 space-y-2.5 rounded-[1.4rem] border border-white/10 bg-black/65 p-2.5 backdrop-blur-xl md:top-24">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2">
          <span className="text-[10px] font-bold uppercase tracking-[0.14em] text-gray-500">Chart health</span>
          ...
          <span className="text-xs text-gray-400">
            {analyticsSectionHealthSummary.total} sections reporting from live admin analytics UI
          </span>
        </div>
