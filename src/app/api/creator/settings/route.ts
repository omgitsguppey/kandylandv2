...
async function readCreatorFanCountSource(creatorId: string, userData: Record<string, unknown>): Promise<{
    ...
    try {
        ...
        recordRouteWarning("creator/settings", "creator_relationships fan source unavailable", relationshipError);
    }
    ...
    try {
        ...
        recordRouteWarning("creator/settings", "creator_relationships_ops fan fallback unavailable", opsError);
    }
...
async function readCreatorDashboardDropCounts(creatorId: string): Promise<{
    ...
    try {
        ...
        recordRouteWarning("creator/settings", "drops dashboard scope source unavailable", error);
        return {
            state: "unavailable",
            contentCount: 0,
            liveDropsCount: 0,
...
async function readCreatorSettingsSource<T>(
    ...
    try {
        ...
        recordRouteWarning("creator/settings", `${name} source unavailable`, error);
        return {
