...
export function normalizeTechnicalFreshnessTerms(value: string): string {
  return value
    .replace(/Operator-confirmed GumDrop revenue smoke was recorded\. Formal provider evidence is still separate\./gu, "Operator-confirmed GumDrop revenue was recorded as product context; provider-backed source activity evidence is still separate.")
    .replace(/Formal provider evidence is still separate\./gu, "Provider-backed source activity evidence is still separate.")
    .replace(/Formal provider smoke evidence is missing\./gu, "Provider-backed source activity evidence is missing.")
    .replace(/Provider smoke:\s*/gu, "Provider-backed source activity: ")
    .replace(/Runtime smoke:\s*/gu, "Deployed route evidence: ")
    .replace(/Attach formal deployed runtime\/provider smoke evidence/giu, "Produce deployed route and provider-backed source activity evidence")
    .replace(/Attach provider smoke evidence/giu, "Produce provider-backed source activity evidence")
    .replace(/Attach redacted provider smoke evidence/giu, "Produce redacted provider-backed source activity evidence")
    .replace(/formal provider smoke/giu, "provider-backed source activity")
    .replace(/formal provider proof/giu, "provider-backed source activity")
    .replace(/formal provider artifact/giu, "provider-backed source artifact")
    .replace(/deployed runtime smoke/giu, "deployed runtime route evidence")
    .replace(/runtime provider smoke/giu, "provider-backed source activity and deployed route evidence")
    .replace(/runtime\/provider smoke/giu, "provider-backed source activity + deployed route evidence")
    .replace(/formal runtime smoke/giu, "deployed runtime route evidence")
    .replace(/formal runtime proof/giu, "deployed runtime route evidence")
    .replace(/formal admin truth evidence gate/giu, "admin source activity evidence lane")
    .replace(/formal admin gate/giu, "admin source activity lane")
    .replace(/formal launch proof/giu, "typed launch evidence artifact")
    .replace(/formal gates/giu, "typed evidence lanes")
    .replace(/formal evidence gates/giu, "typed evidence lanes")
    .replace(/formal evidence gate/giu, "typed evidence lane")
    .replace(/formal evidence/giu, "typed evidence artifact")
    .replace(/formal proof/giu, "typed evidence artifact")
    .replace(/not formal provider proof/giu, "not provider-backed source evidence")
    .replace(/Attach real redacted provider smoke evidence/gu, "Produce provider-backed source activity evidence")
    .replace(/Attach a redacted production admin truth sample/gu, "Produce redacted admin source activity evidence")
    .replace(/Attach redacted production admin truth sample evidence/gu, "Produce redacted admin source activity evidence")
    .replace(/provider-backed site activity evidence/giu, "provider-backed source activity evidence")
    .replace(/provider-backed site activity/giu, "provider-backed source activity")
    .replace(/admin source activity sample evidence/giu, "admin source activity evidence")
    .replace(/admin source activity sample/giu, "admin source activity evidence")
    .replace(/without clearing source evidence lanes/gu, "without clearing source evidence lanes")
