...
const FEATURE_SEEDS: FeatureSeed[] = [
  ...
  },
  {
    ...
    routePaths: ["src/app/dashboard/support", "src/app/api/support", "src/app/faq", "src/app/api/bug-reports"],
    uiSurfaces: ["support", "FAQ", "bug reports"],
    scoreDimensionsAffected: ["runtimeHealth", "evidenceCompleteness"],
    userCreatorAdminParityRequirements: ["Support telemetry must stay scoped to user-owned support truth and admin inbox evidence."],
    rolloutStatus: "source_ready",
    legacyOrphanRecoveryStatus: "orphan_registry_tracked",
    metricIds: ["behavior_signals"],
  },
