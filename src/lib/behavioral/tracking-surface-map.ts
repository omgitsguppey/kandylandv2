...
export const TRACKING_SURFACE_MAP: TrackingSurfaceDefinition[] = [
  ...
  },
  {
    ...
    requiredFields: ["source_component", "query_length", "query_hash", "raw_query_stored"],
    requiredIdentityFields: ["anonymousVisitorId or actorUserId", "sessionId"],
    allowedWhenTrackingDisabled: false,
    persistenceLane: "analytics_event_facts with redacted query length/hash/category only",
    aggregationLane: "search/discovery summaries and admin debug evidence",
    uiConsumer: "Drops search and discovery filters",
    adminConsumer: "Admin Debug search/discovery lane and beta behavior evidence",
    enabledBehavior: "Persist debounced search intent, zero-result, result-click, and failure signals without raw query text.",
    disabledBehavior: "Suppress optional search behavior telemetry; never send raw query text to broad telemetry.",
  },
  {
