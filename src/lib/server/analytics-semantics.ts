...
function buildSemanticDeltaFromTelemetryRollup(input: {
  eventName: string;
  ...
  return buildTelemetrySemanticDelta({
    ...
    sessionWatchSeconds: input.sessionWatchSeconds,
    clickCount: asNumber(input.params?.click_count),
  }, {
    preferSemanticPageSignals: true,
    preferSemanticClickSignals: true,
  });
}
...
function writeRollup(
  batch: WriteBatch,
  timestamp: number,
  context: ReturnType<typeof resolveAnalyticsSemanticContext>,
  delta: SemanticMetricDelta,
  sourceKey: string,
) {
  ...
  batch.set(
    rollupRef,
    {
      ...
      viewCount: FieldValue.increment(delta.viewCount ?? 0),
      viewDurationMs: FieldValue.increment(delta.viewDurationMs ?? 0),
      engagedViewCount: FieldValue.increment(delta.engagedViewCount ?? 0),
      passiveViewCount: FieldValue.increment(delta.passiveViewCount ?? 0),
      bounceCount: FieldValue.increment(delta.bounceCount ?? 0),
      exitCount: FieldValue.increment(delta.exitCount ?? 0),
      clickCount: FieldValue.increment(delta.clickCount ?? 0),
      hoverCount: FieldValue.increment(delta.hoverCount ?? 0),
      signInCount: FieldValue.increment(delta.signInCount ?? 0),
      returnCount: FieldValue.increment(delta.returnCount ?? 0),
      logoutCount: FieldValue.increment(delta.logoutCount ?? 0),
      watchSecondsTotal: FieldValue.increment(delta.watchSecondsTotal ?? 0),
      watchSessionCount: FieldValue.increment(delta.watchSessionCount ?? 0),
      pagesVisited: FieldValue.arrayUnion(delta.pagePath || context.pagePath),
      sourceKeys: FieldValue.arrayUnion(sourceKey),
      semanticParams: buildAnalyticsSemanticParams({
        pagePath: context.pagePath,
        dropId: context.dropId,
        dropCategory: context.dropCategory,
      }),
...
export async function recordSemanticRollupFromTelemetryEvent(input: {
  timestamp: number;
  eventName: string;
  params?: Record<string, unknown>;
  sourceKey: string;
  ...
  const pagePath = asString(params.page_path) || getLegacyPagePathForEvent(input.eventName);
  const dropId = asString(params.drop_id);
  const dropCategory = asString(params.drop_category);
  const delta = buildSemanticDeltaFromTelemetryRollup({
    eventName: input.eventName,
    params,
    pagePath,
    dropId,
    dropCategory,
    durationMs: asNumber(params.duration_ms),
    watchSeconds: asNumber(params.watch_seconds),
    sessionWatchSeconds: asNumber(params.session_watch_seconds),
  });
  ...
  writeRollup(batch, input.timestamp, context, delta, input.sourceKey);
  await batch.commit();
}

export async function recordSemanticRollupFromGuestEvents(input: {
  timestamp: number;
