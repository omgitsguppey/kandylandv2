...
export function buildCreatorOnboardingDiagnostics(input: {
    ...
    onboardingByUser.forEach((canonical, userId) => {
        ...
        if (isLiveCreatorSource(sourceSnapshot) && !hasRecord(userRaw?.creatorSettings)) {
            pushIssue({
                ...
                detail: "Live creator settings are missing. Synthetic/internal creator classification only changes legal evidence handling and must not downgrade required fan experience settings.",
                link: `/admin/user/${userId}`,
    ...
    const summary: CreatorOnboardingDiagnosticSummary = {
        ...
        settingsIssueCount: issues.filter((entry) => entry.rosterWarning === "Settings missing" || entry.rosterWarning === "Settings need review").length,
