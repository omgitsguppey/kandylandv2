...
export type DirtyFileClassification =
  ...
  | "source_agreement_failure_classification_required"
...
export function classifyEventTranslationDirtyFile(path: string): DirtyFileClassification {
  ...
  if (normalized === "src/lib/analytics/admin-analytics-source-hierarchy.ts" || normalized === "src/lib/debug/debug-cockpit-batch29-analytics-source-hierarchy.ts") return "real_source_change_needs_review";
  if (normalized === "agent/state/admin-analytics-source-hierarchy.generated.json" || normalized === "agent/state/debug-cockpit-batch29-analytics-source-hierarchy.generated.json") return "current_generated_artifact_to_commit";
  if (normalized === "docs/agent-truth/admin-analytics-source-hierarchy.md" || normalized === "docs/agent-truth/debug-cockpit-batch29-analytics-source-hierarchy.md") return "documentation_artifact_expected";
  if (normalized === "tests/unit/admin-analytics-source-hierarchy.spec.ts" || normalized === "tests/unit/debug-cockpit-batch29-analytics-source-hierarchy.spec.ts") return "test_artifact_expected";
