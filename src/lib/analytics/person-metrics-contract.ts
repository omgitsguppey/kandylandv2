...
export const PERSON_METRIC_IDS = [
  ...
  "chat_actions",
  "daily_task_views",
  "daily_task_guidance_interactions",
  "daily_task_starts",
...
export const PERSON_METRIC_DEFINITIONS: PersonMetricDefinition[] = [
  metric({
    ...
    sourceOfTruth: "analytics_sessions with active/idle/hidden split; page-open duration alone is not active session time",
  ...
  metric({
    ...
    sourceOfTruth: "analytics_event_facts and creator relationship funnel telemetry",
    consentEligibility: { depth: "minimal_product", allowedModes: MINIMAL_MODES },
  ...
  metric({
    id: "search_discovery_actions",
    label: "Search discovery actions",
    eventNames: [
      "search_focused",
    ...
    sourceOfTruth: "redacted search/discovery telemetry with query length/hash/category only",
    consentEligibility: { depth: "minimal_product", allowedModes: MINIMAL_MODES },
    materializer: "person_metrics.search_discovery_actions",
    debugOwner: "analytics",
    scoreEvidenceImpact: "behavior_confidence",
    legacyRecoveryMapping: { action: "normalize_candidate", confidence: "weak", source: "legacy search and discovery events" },
    minimumConfidence: "weak",
  }),
  metric({
    id: "drop_opens",
  ...
  metric({
    id: "drop_unlocks",
    label: "Drop unlocks",
    eventNames: ["drop_unlocked", "entitlement_granted", "unlock_drop_success"],
    sourceOfTruth: "server unlock entitlement facts; payload reveal is tracked separately as unwrap",
    consentEligibility: { depth: "necessary_product", allowedModes: NECESSARY_MODES },
    materializer: "person_metrics.drop_unlocks",
    debugOwner: "analytics",
  ...
  metric({
    ...
    sourceOfTruth: "creator relationship facts and relationship route telemetry",
    consentEligibility: { depth: "minimal_product", allowedModes: MINIMAL_MODES },
  ...
  metric({
    ...
    sourceOfTruth: "chat telemetry event facts, private media access decisions, and guarded creator message thread records; raw transcript content and private asset URLs are drilldown-only",
    consentEligibility: { depth: "minimal_product", allowedModes: MINIMAL_MODES },
  ...
  metric({
    id: "daily_task_guidance_interactions",
    label: "Daily task guidance interactions",
    eventNames: [
    ...
    sourceOfTruth: "task guidance UI telemetry; guidance completion is supporting evidence and does not grant rewards by itself",
    consentEligibility: { depth: "minimal_product", allowedModes: MINIMAL_MODES },
    materializer: "person_metrics.daily_task_guidance_interactions",
    debugOwner: "retention",
    scoreEvidenceImpact: "evidence_completeness",
    legacyRecoveryMapping: { action: "normalize_candidate", confidence: "weak", source: "legacy task guidance aliases" },
    minimumConfidence: "weak",
  }),
  metric({
  ...
  metric({
    ...
    sourceOfTruth: "runtime media/drop watch sessions only; page duration is not a current-person metric",
