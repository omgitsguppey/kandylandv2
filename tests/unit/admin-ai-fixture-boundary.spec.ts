...
describe("admin AI local fixture boundary", () => {
  it("labels Cover Ops as source_missing instead of live admin AI evidence in local fixture mode", () => {
    expect(pageSource).toContain("data-admin-ai-fixture-boundary=\"true\"");
    expect(pageSource).toContain("Cover Ops data is source_missing in local review");
  ...
  it("skips admin AI cover reads and writes for local fixture sessions", () => {
    ...
    expect(hookSource).toContain("Cover Ops settings are source_missing in local review");
  ...
  it("skips nested description operations reads and writes for local fixture sessions", () => {
    ...
    expect(descriptionSource).toContain("Description operations are source_missing until a real admin session loads verified AI evidence");
