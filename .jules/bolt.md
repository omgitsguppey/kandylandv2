...
## 2026-04-26 - Consolidating Loops and Primitive Dependencies in React Hooks
**Learning:** In hot-path hooks like `useDrops`, iterating the same large data array (`swrDrops`) multiple times for different purposes (e.g., calculating next expiry timestamps in a `useEffect` and filtering elements in a `useMemo`) adds unnecessary CPU load. Furthermore, complex logic inside a `useEffect` often relies on array or object dependencies, which can cause subtle dependency array issues or unnecessary effect evaluations.
**Action:** Consolidate related data transformations into a single-pass `useMemo` block that returns an object containing multiple primitive results. In `src/hooks/useDrops.ts`, we combined entity filtering and the expiry timestamp calculation into one `useMemo` loop, returning `{ clientDrops, nextExpiryMs }`. The dependent `useEffect` was then updated to depend strictly on the simple `nextExpiryMs` primitive, improving performance and effect stability.
