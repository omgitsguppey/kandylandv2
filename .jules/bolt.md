...
## 2026-05-19 - Array O(N^2) Complexity in NextJS API Routes
**Learning:** High performance degradation occurs in large scale data processing operations within API endpoints like `src/app/api/admin/debug/route.ts` when `Array.prototype.find()` or `Array.prototype.filter()` is used inside mapping loops over large collections (e.g. mapping telemetry events and finding corresponding tasks). This creates an O(N^2) complexity that scales poorly under load.
**Action:** Always precompute a `Map` prior to iterating arrays. Use Map lookups `map.get(key)` inside the loop, turning the O(N^2) complexity into an O(N) lookup. Make sure to only map the first matching element using `if (!map.has(key)) map.set(...)` when emulating the exact behavior of `Array.prototype.find()`.
