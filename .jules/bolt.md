...
## 2024-05-18 - Replacing multi-pass filter/reduce with single pass loop
**Learning:** Found multiple instances where arrays are iterated multiple times using `.filter(...).reduce(...)` to compute aggregations. This incurs an O(N*M) performance penalty where N is the array length and M is the number of aggregations, creating unnecessary intermediate arrays and increasing GC pressure.
**Action:** Replace sequential or chained `.filter()`, `.map()`, and `.reduce()` operations with a single-pass `for...of` loop to track multiple states simultaneously, improving both CPU and memory performance.
