...
## 2026-07-14 Validator-owner retirement and score-evidence authority memory

Only current-source reports with an active canonical consumer may participate in readiness scoring or clear a source gate. `check:settings-debug-validator-authority` is the sole active owner of the six-component settings-health contract; `check:settings-health-status-cleanup` remains compatibility history only with `historical_evidence_only` and every gate disabled. `check:route-sample-freshness-classifier` owns active route-sample freshness, so `check:stale-route-sample-classification` and its report family are retired historical batch output. `check:admin-summary-lane-status-classifier` owns loaded, no-sample, failed, and stale Admin summary classification, so `check:user-management-status-truth` and its synthetic report family are retired. Remove retired aliases, wrappers, tests, artifacts, docs, and generated authority-map entries together after consumer and score-gate searches prove no active dependency. Archive evidence must never count as source, runtime, provider, payment, or admin-truth proof.
...
## 2026-07-14 Legacy semantic retirement memory

Legacy registry stages describe the forbidden behavior, not whether every file named by the record has been deleted. A record may be `removed` while its blocked-reference guards remain active and its listed files host the canonical replacement. Promote a legacy semantic to `removed` only after the owning scanner reports zero blocked runtime references and focused owner tests prove the replacement path; keep the blocked strings as regression guards. Use `unused` when a fallback file remains but repository imports prove it has no active consumer. Do not mark active infrastructure such as the Admin users realtime route removed merely to improve the legacy score.
...
## 2026-05-27 Identity tracking and handoff truth memory

When individual user analytics are wrong, inspect the full identity chain: guest id creation, session id, event envelope actor, auth state, identity link, signed-in user id, linked person id, person metric hydration, journey attribution, debug lane, and panel display. Do not fix only global metrics. Global metrics proving activity does not prove user-level metrics. Every event must declare whether it counts globally, for guest, for signed-in user, for linked person, for creator role, or not at all. Guest-to-user handoff must preserve journey continuity without double-counting. Linked guest and signed-in events representing the same action should count once globally and once for the linked person. Missing individual user data is not zero. Show collecting, source_missing, bridge_missing, materializer_missing, or permission_blocked unless a bounded source window proves zero. Identity/session 4xx errors are not generic failures. Expired session, missing guest id, stale user id, role drift, identity link conflict, and malformed handoff payload must be classified, non-retryable where appropriate, and debug-visible. `check:identity-chain-contract`, `check:guest-user-handoff-repair`, `check:individual-user-metric-truth`, `check:analytic-algorithm-truth-audit`, `check:identity-handoff-4xx-policy`, `check:user-tracking-live-evidence`, and `check:identity-tracking-memory-writeback` validate the lane.

User/guest index execution has one owner chain: ingest atomically persists `behavioral_timeline_facts` plus deterministic `user_index_materializer_requests`; `src/lib/server/user-index-materializer.ts` is the unique normalization/publication algorithm; the CRON-authenticated internal route is invoked by the single-instance bounded scheduled Function. The lane defaults off, shadow publication requires two distinct clean current-source windows before active, and rollback disables the mode without deleting facts or receipts. Source/config checks prove wiring only; deployment, secrets, index rollout, scheduler invocation, shadow cleanliness, admin truth, and production person metrics remain external evidence.

Materializer and identity zero-proof is fail-closed. A subject read that reaches the bounded fact cap is potentially truncated and must emit `materializer_subject_truncated`: shadow mode may retain explicitly partial evidence, but the window is not clean, active publication must reject it, and the scheduled Function must enforce the same receipt and issue-code semantics as the canonical writer. Identity evidence is eligible only when lineage records carry the truthful current owner-key version `fnv1a32_guest_session_user_base36_v1`; missing, legacy, or unsupported versions are rejected before materialization and surfaced as bridge evidence gaps. `proven_zero` additionally requires the current materializer version, a matching source fingerprint, and ordered bounded-window/publication timestamps that are neither future nor older than the 15-minute freshness tolerance; otherwise the surface remains collecting, bridge-missing, or materializer-missing/stale, never zero.

## 2026-05-27 Creator drop workflow repair memory

When a creator feature is reported broken, inspect the entire workflow chain from creator UI to backend action, permission check, stored status, admin queue, approval decision, user visibility, telemetry, debug, and 4xx handling. Do not fix only the visible button. Creator-submitted drops must remain creator-visible but user-hidden until admin approval. Do not allow creator submissions to enter user-facing rotation without approval. Every creator workflow must expose creator-facing status and admin-facing status. Silent success, silent failure, and hidden pending states are product bugs. Creator feature 4xx errors need user-readable recovery states. Validation errors, permission denials, missing creator settings, stale draft IDs, and admin approval conflicts must be non-retryable and diagnostic-safe. Before adding a new creator route or component, search for existing creator settings, creator drop manager, admin drops, drop approval, and creator monetization contracts. Reuse and consolidate instead of adding parallel creator systems. `check:creator-drop-workflow-contract`, `check:creator-drop-submit-repair`, `check:creator-drop-admin-approval-parity`, `check:creator-drop-4xx-policy`, `check:creator-feature-parity-map`, `check:creator-drop-live-evidence`, and `check:creator-drop-memory-writeback` validate the lane.

## 2026-05-27 Privacy data lifecycle consolidation memory

Before adding telemetry, analytics, behavioral intelligence, admin debug, chat, media, notification, or journey tracking, check the canonical privacy/data lifecycle policy. Do not collect, store, export, or display data unless the consent and privacy class allow it. Consent denied does not mean "track nothing." It means only allowed operational/security/minimal analytics may be collected according to the privacy policy. Deep behavioral tracking requires explicit allowed state. Delete account, data export, support/report issue, and admin debug must all use the same redaction and retention policy. Do not create one-off redaction or deletion logic inside routes or components. Behavioral/user journey records must have privacy class, retention class, export eligibility, delete eligibility, and redaction policy. Unknown legacy data must not be exported as exact user truth. Never use manual screenshots or admin panels to expose raw sensitive data. Redacted summaries are the default. Raw drilldown requires explicit admin permission and audit reason. `check:privacy-data-lifecycle-contract`, `check:consent-interpreter-consolidation`, `check:data-export-canonicalization`, `check:delete-account-retention-policy`, `check:admin-redaction-drilldown-policy`, `check:support-account-safety-consolidation`, and `check:privacy-data-memory-writeback` validate the lane.

## 2026-05-27 4xx route and Treasury consolidation memory

Classify 4xx errors before fixing them. Validation, auth, permission, not found, conflict, rate limit, and permanent client errors must not be treated as retryable server failures. Never let 4xx diagnostics create retry storms, duplicate warning writes, or fake P1 runtime issues; roll up repeated 4xx failures by route, error class, actor class, and hourly fingerprint. Before adding finance, wallet, GumDrop, revenue, transaction, entitlement, or cost logic, check the Treasury owner map. Do not create a new ledger, revenue summary, balance calculation, or source-of-funds rule outside the canonical Treasury structure. Treasury must separate appropriations, obligations, outlays, receipts, balances, entitlements, refunds, adjustments, reconciliation, and audit exceptions. Do not mix source-of-funds, revenue confidence, and user wallet display math. Treasury reports must be compact, redacted, reconcilable, and confidence-labeled. Unknown legacy money data must never display as exact revenue, exact balance, or exact entitlement. Every finance-like panel or metric must explain whether it is ledger truth, provider truth, operator-confirmed, inferred, pending, reversed, or unknown legacy. `check:route-4xx-taxonomy`, `check:route-4xx-mitigation`, `check:treasury-structure-contract`, `check:treasury-consolidation-map`, and `check:treasury-reconciliation-engine` validate the lane.
...
## 2026-05-27 Config infra consolidation memory

Config, deployment, environment, CI, package-script, middleware, Firebase rules, and dependency tooling changes are release-risk changes, not routine cleanup. Do not treat passing source tests as deployment readiness. Deployment readiness requires current-head score artifacts, release notes, env contract validation, security/rules validation, PR hygiene, rollback status, and explicit operator evidence classification. Do not add package scripts, validators, or generated artifacts without checking for existing scripts that already own the same lane; consolidate aliases and retire stale scripts instead of growing the command surface. Every config or deployment change must declare owner, environment, expected command, safety class, cost class, and rollback behavior. Missing env vars, stale release artifacts, or unclassified open PRs must block release readiness. Never touch payment runtime, GumDrop math, provider callbacks, Firebase/security rules, middleware, or deployment configs casually. Any change in those lanes requires targeted validators and release note classification. Before editing config, deployment, env, CI, package scripts, middleware, Firebase rules, or dependency tooling, classify the lane and read existing policy. Do not add a new check, workflow, or validator if an existing script owns the same exit gate. Config and deployment changes are release-risk changes. They require env contract, security/rules classification, cost class, rollback behavior, and current-head score/release packet refresh. Dependency PRs are not cleanup. Treat them as a separate dependency window unless security-required. Never merge broad dependency bundles as part of beta-exit hardening. Generated artifacts must be compact by default. Do not store thousands of static lines when a summary and drilldown can be derived. `check:config-infra-memory-writeback` validates the lane.
...
Platform Economy is now the canonical admin GumDrops treasury and commerce control surface. `src/lib/platform-economy.ts` owns the base `$1 = 100 GD` anchor, the `$0.50 / 100 GD` warning floor, package/promo/offer/redemption/drift types, paid-vs-reward source-of-funds helpers, promo normalization, and warning builders. `src/lib/server/platform-economy.ts` derives current package truth from the fixed package catalog or admin config docs, aggregates treasury totals with Firestore sum aggregates, reads bounded wallet rows and recent purchase redemptions, and compares package/promo/wallet/revenue/ledger drift. `src/lib/server/platform-economy-mutations.ts` provides admin-only, trusted-origin, versioned draft mutations for packages, promos, and offers with below-floor override reasons and inactive-by-default behavior. `/api/admin/economy/*` now exposes treasury, packages, promos, offers, redemptions, and drift slices, while `/admin/economy` renders the compact GumDrops Commerce Control Center. PayPal capture now writes forward-looking package/source-of-funds metadata (`packageId`, `promoId`, `offerId`, `priceUsdBeforeDiscount`, `priceUsdPaid`, `sourceOfFundsBreakdown`, `idempotencyKey`, `orderId`) without changing live pricing or balance mutation logic.

## 2026-05-05 Codex native auth readiness

KandyDrops now has a read-only Codex auth readiness lane. `src/lib/devops/auth-surface-contract.ts` owns auth surface statuses, blocked mutation commands, report shape, and the Workload Identity Federation bootstrap plan. `npm run check:codex-auth` probes GitHub CLI, GitHub repo metadata, gcloud, Firebase CLI, bq, Cloud SQL/Data Connect, App Engine, Cloud Run/App Hosting, Scheduler/Functions, Artifact Registry, PayPal config, and PostHog/GA config without printing secrets or mutating cloud resources. `agent/state/codex-auth-readiness.generated.json` currently reports `repo_only_ready`: GitHub CLI is authenticated as repo admin, while `gcloud`, `firebase`, and `bq` are missing from PATH. `npm run plan:cloud-auth-bootstrap` prints WIF setup commands, `.github/workflows/cloud-readiness-smoke.yml` is manual-only with `contents: read` and `id-token: write`, and `npm run check:codex-auth-readiness` validates the lane.

## 2026-05-05 Behavioral math calibration

KandyDrops behavioral intelligence now uses source-truthed prediction math instead of broad aggregate affinity scoring. `src/lib/behavioral/behavioral-math-calibration.ts` owns source reliability weights, truth score, engagement/value score helpers, drop recommendation scoring, prediction outputs, surface objectives, and ML activation rules. Recommendation ranking now emits `pPurchase7d`, `pUnlock24h`, `pWatchComplete`, `pReturn7d`, `pCreatorFollow`, and `pNegativeFeedback`, with deterministic and ML artifact scores sharing the same product-goal formula. Behavioral Functions materialization and admin user rollups now treat server purchases, server entitlement unlocks, and watch-session rollups as first-class truth, while client UI events and `legacy_page_duration` stay supporting/degraded context. `npm run validate:behavioral-predictions` writes `agent/state/behavioral-math-calibration.generated.json`; `npm run check:behavioral-math-calibration` validates formulas, truth sources, prediction fields, admin visibility, and ML sample/baseline guardrails.
...
Unsafe synthetic/simulative Admin view-as semantics are retired as `blocked` plus `removed`; the active replacement is the read-only/local Admin creator projection. Generic `includeInUserBehavior: true` belongs to legitimate signed-in-user, guest, and canonical fixture lanes and is no longer owned by the legacy substring scanner. `check:admin-projection-analytics-exclusion` owns the precise invariant that Admin projection payloads identify `actorAdminId`/`performedAs`, remain local projection, emit write-block diagnostics, and always set `includeInUserBehavior: false`. Removed or canonical legacy records carry no overdue multiplier or overdue next action, while their remaining blocked signatures still prevent unsafe semantics from re-entering source.
