...
const sharedContent = `
import { Activity, ChevronDown, ChevronUp, Radio, ShieldAlert, Sparkles, Terminal } from "lucide-react";
...
const writeTab = (comp, startIdx, endIdx) => {
    const tabBlock = lines.slice(startIdx + 1, endIdx).join('\n');
    const tabFile = `
...
import { AdminTruthDashboards } from "./AdminTruthDashboards";
...
// 0-indexed slicing based on Powershell check
// Now: 681 - 940 -> lines(680) is '{activeTab === "now" ? (', lines(939) is ') : null}'
writeTab('DebugNowTab', 680, 939);
writeTab('DebugActionsTab', 940, 1042);
writeTab('DebugMonitoringTab', 1043, 1461);
writeTab('DebugAITab', 1462, 1596);
writeTab('DebugAdvancedTab', 1597, 2161);
...
import { DEBUG_TABS, StatCard, formatRelative, getPipelineStatusLabel, formatWindowHours } from "./components/DebugShared";
