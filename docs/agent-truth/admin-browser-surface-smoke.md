...
- For direct in-app Browser audits without Playwright, start the local dev server with NEXT_PUBLIC_ENABLE_ADMIN_UI_TEST_SESSION=1, open /admin?adminUiTestSession=1 once to seed the bounded local fixture session, then navigate admin routes normally; this proves local route rendering only.
