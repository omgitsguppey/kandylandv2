...
The public beta score is action-first. `operatorDecision` is the canonical operator projection: resolve `sourceFixes`, then `sourceVerification`, then `evidenceRefresh`, then `externalProof`, and finally `ownerReview`. Source readiness and release readiness are separate truths. `launch_ready` is valid only when source readiness is `ready`, launch blockers are empty, owner-review actions are empty, and the typed release decision is ready. A non-blocking optional lane is an advisory; it must not become a cap or lower the release decision.

The overall/health composite is backward-compatible diagnostic trend context only. It is not a work target, release decision, or permission to average away a failed source or evidence gate. Operators act on the typed queues and the launch decision. Evidence score is normalized across positive-weight gates to a 0-100 diagnostic value; zero-weight derived or advisory lanes cannot inflate it. No evidence gate may report `Ready` while also setting `blocksLaunch=true`.
