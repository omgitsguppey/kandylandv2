...
- Repairs available now first dedupes by `sourceCollection`, canonical source path, repair kind, and missing-context reason. It then groups repeated inspect-only notification and server drop-click source-context records into compact review cards. Grouped cards show affected record count, first five source paths by default, collapsed remaining paths, duplicate proposal ids, first/last seen UTC, source context state, missing context fields, and actionability. Inspect-only proposals are review items, not actionable repairs, and must not show Apply as the primary action.
- Bug reports to triage uses `BugIntakeTriageSummary` and `BugReportTriageCard`. Loaded sample count, last-seven-day intake, older backlog, and needs-triage count are separate truths. Known status, severity, breadcrumb, diagnostic, rollout, and timestamp values must render as NEW/REVIEW/LOADED/INFO/BACKLOG, not WAIT. Older loaded reports remain visible under "Older backlog" when `Last 7d` is lower than the loaded sample.
- Tracked route runtime uses `RouteRuntimeSummaryTruth` and `RouteRuntimeRecordTruth`. Tracked, observed, sample, average latency, and cluster values are loaded inventory/info when numeric. Native and compatibility chat rollups must label fail, stale, and unseen counts instead of rendering ambiguous triples. Route rows separate coverage, freshness, last result, counts, and latency state; a fresh healthy route with historical slow samples is "healthy with latency review", not WAIT or ERROR.
- Unseen route runtime records must never render fake success or 0ms latency. Before a route has samples, `lastResult` is `no_sample`, latency fields render as unavailable/unknown, and route risk class (`critical`, `high`, `normal`, `optional`, `legacy`) controls whether the unseen route is review or quiet. Current result, latency history, and error history stay separate for observed routes.
- Recent transactions are loaded operational evidence, not a ledger mutation surface. Loaded counts and feed-window labels render as LOADED/INFO, never WAIT. Rows use safe admin identity enrichment (username, display name, allowed email prefix, then short UID fallback), link to `/admin/user/[userId]`, preserve full UID only in collapsed details/debug attributes, show UTC timestamps, classify transaction type/source, and display amounts with units such as `+10 GD`.
- Queue runtime continuity separates scheduler heartbeats, dispatch outcomes, runtime warnings, and legacy adapters. Dispatch outcome rows are enriched from the bounded outcome drop IDs with drop title, creator label, scheduled UTC time, last outcome UTC, outcome/error state, admin links, and collapsed raw scheduler keys. Missing heartbeat records with loaded outcomes must explain that heartbeat evidence is missing while outcome evidence exists.
- Admin session + config readiness confirms admin session identity and prerequisite configuration presence only. GA property, VAPID, Database URL, and navigation signing config must render as config-present/config-missing and runtime-not-verified unless an explicit runtime self-check ran. Full admin UID/email stay collapsed under session details, and this card must not imply external GA, push, database, or signing health.
- Recent event flow uses `RecentEventFlowRow` display truth. Loaded actor, surface, findings, and eval eligibility values render as loaded/info/review, never WAIT. Rows show `createdAtUtc`, age, freshness, event context, source, eval eligibility reason, and required missing inputs only when they are required for that context. Repeated low-risk events are grouped within a short window by event, actor, surface, source, and context. `identity_linkage` is informational and not behavior-ranked. `background_task_engine`, `background_ledger`, `notification_system`, `server_system`, and materialized relationship events do not require foreground route/session fields unless explicitly promoted to user behavior scoring. Notification system warnings/successes and system drop clicks must be review/excluded, not critical, solely for missing route/session.
