...
- Recent event flow uses `RecentEventFlowRow` display truth. Loaded actor, surface, findings, and eval eligibility values render as loaded/info/review, never WAIT. Rows show `createdAtUtc`, age, freshness, event context, source, eval eligibility reason, and required missing inputs only when they are required for that context. Repeated low-risk events are grouped within a short window by event, actor, surface, source, and context. `identity_linked` is informational and not behavior-ranked. Background/server/system/materialized relationship events do not require foreground route/session fields unless explicitly promoted to user behavior scoring.
