...
Scope: Phase 1 admin truth inventory after deploy-readiness repair and external deploy-check classification.
Evidence class: source inspection plus generated snapshot review. This document is not runtime/provider/admin proof.
Current code version reviewed: `734b6d998333eb2482fce9a6695ea89b35f63977`.

## Current Gate State

- Deploy source baseline: `npm run typecheck`, `npm run build`, `npm run check:release-notes`, `npm run check:beta-score`, and `npm run check:current-beta-exit-status` passed for deploy-readiness commit `a8313045e`.
- External check boundary: `734b6d99` classifies Firebase App Hosting, Google Cloud Build, and Graphite as external check providers. Source validators cannot clear those app/provider checks.
- Current local source validators after `734b6d99`: `npm run check:ci-release-discipline`, `npm run agent:test -- tests/unit/ci-release-discipline.spec.ts`, `npm run typecheck`, `npm run check:release-notes`, and `npm run check:environment-deployment-truth` passed.
- Current beta/generated evidence is stale by head after the latest internal commits. `current-beta-exit-status` and `public-beta-score` still point at `6579bbf...`; `admin-browser-surface-smoke` points at `fa4d06a...`; admin truth samples point at June 3 commits.
...
- PR inventory and branch-protection edits are external in this session. Local `gh` is unavailable, the GitHub connector was expired/rate-limited, and unauthenticated GitHub REST was rate-limited. Do not interpret that as zero open PRs or healthy branch checks.
...
| Release / Readiness | Admin Debug and release docs | `src/lib/release-readiness/*`, `src/lib/config-hardening/*` | Source validators and Cloud Build/App Hosting external providers | Public beta score, release notes, CI discipline, generated report authority | Source-owned validators plus external proof lanes | `ci-release-discipline` current at `734b6d99`; beta score/current-exit stale by head | External provider checks can stall independently of source config | Firebase App Hosting, Cloud Build trigger, provider smoke, admin truth sample | Refresh current-head artifacts only in owned final/signoff slice; reauthorize external providers outside source checks. |

## Evidence Boundary Snapshot

| Artifact | Current state | What it proves | What it does not prove | Next action |
| --- | --- | --- | --- | --- |
| `ci-release-discipline` | Current at `734b6d99`, passed | GitHub workflows, Cloud Build YAML, App Hosting/Cloud Build/Graphite boundaries are classified | Actual Firebase/GCP/GitHub provider health | Reauthorize/restart external providers or remove stale Graphite required check outside source. |
| `environment-deployment-truth` | Passed after `734b6d99` | Source deploy config is registered and sane | App Hosting rollout success or provider secret presence | Attach external App Hosting rollout evidence. |
| `current-beta-exit-status` | Stale by head, points at `6579bbf...` | Historical proof-lane classification | Current deploy readiness or runtime proof | Refresh only in a final/signoff slice that owns current-head artifacts. |
| `public-beta-score` | Stale evidence, points at `6579bbf...` | Historical public beta score | Current release score | Refresh with `score:beta` only when release/readiness slice owns it. |
...
6. Release / Readiness: refresh current-head score/exit artifacts only when final signoff owns them; external provider checks stay external.
