...
- 1. beta-cap-ready-with-smoke-required-admin-truth-sample-evidence-2: 51 (blocked_formal_evidence) - Attach a redacted first-party admin truth sample before clearing the formal admin truth evidence gate.
- 2. beta-cap-visual-qa-required-visual-manual-smoke-0: 51 (needs_operator_ui_confirmation) - Attach targeted manual or screenshot evidence before clearing visual/manual smoke.
- 3. beta-cap-runtime-unverified-runtime-provider-smoke-1: 30.67 (blocked_formal_evidence) - Attach formal deployed runtime/provider smoke evidence before clearing this beta gate.
