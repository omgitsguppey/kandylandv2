...
const oldCta = `    const ctaButton = isUnlocked ? (
        <div className="w-full flex items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-brand-purple bg-gradient-to-r from-brand-purple to-purple-500 px-3 py-2 text-xs font-bold text-white shadow-[0_0_10px_rgba(164,118,255,0.2)] transition-all">
        ...
        <div className="w-full flex items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-brand-purple bg-gradient-to-r from-brand-purple to-purple-500 px-3 py-2 text-xs font-bold text-white shadow-[0_0_10px_rgba(164,118,255,0.2)] transition-all">
            ...
            {!user ? "Create Profile to Unwrap" : !canAfford ? "Get more GumDrops" : \`Unwrap for \${drop.unlockCost} GD\`}
        </div>
    );`;

const newCta = `    const ctaButton = isUnlocked ? (
        <div className="w-full flex items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-brand-purple bg-gradient-to-r from-brand-purple to-purple-500 px-3 py-1.5 text-[10px] md:text-xs font-bold text-white shadow-[0_0_10px_rgba(164,118,255,0.2)] transition-all">
...
code = code.replace(/\r\n/g, "\n");
const oldCtaNorm = oldCta.replace(/\r\n/g, "\n");
const old916Norm = old916.replace(/\r\n/g, "\n");
const oldStdNorm = oldStd.replace(/\r\n/g, "\n");
const oldFullStdNorm = oldFullStd.replace(/\r\n/g, "\n");

let error = false;
if (code.includes(oldCtaNorm)) { code = code.replace(oldCtaNorm, newCta.replace(/\n/g, "\r\n")); } else { error = true; console.error("CTA mismatch"); }
