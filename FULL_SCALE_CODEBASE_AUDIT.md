...
## 2024-05-18 - Added proper loading states to unwrap buttons
...
Problem fixed:
- Missing `aria-busy` and `aria-hidden` attributes for unwrap buttons and loading spinners, making loading states inaccessible to screen readers.

Hardening applied:
- Added `aria-busy={unlocking}` to unwrap buttons and `aria-hidden="true"` to `<Loader2>` spinners in DropCardCta, DropPreviewModal, and LockedDropPreviewView to ensure proper asynchronous state announcements.

Verification:
- Checked visual structure using tests
- Verified using code review
