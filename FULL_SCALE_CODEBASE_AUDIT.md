...
\n## 2026-05-22 Purchase Modal GumDrop Economy Accuracy Update\n\n- Scope: Addressed visual double-counting of bonus GumDrops in `src/components/PurchaseModal.tsx` to prevent misrepresentation of purchased vs. non-purchased value.\n- Finding: **Visible value claim mismatch**. The UI displayed the total GumDrops (base + bonus) alongside the explicitly labelled bonus amount, falsely implying the total value received was `base + bonus + bonus`. This mixed purchased and non-purchased value misleadingly.\n- Resolution: Replaced `pkg.drops.toLocaleString()` and `customDrops.toLocaleString()` with their respective `deriveGumdropEconomics(...).paidGumDrops` values. This restricts the primary number to the true purchased amount, mirroring the `+ [bonus] paid bonus GD` display correctly and maintaining clear source-of-funds truth.\n- Blast Radius: Localized to the `PurchaseModal` component's presentation layer. Protected payment and ledger internals remain untouched.
