...
- [x] `docs/agent-truth/device-layout-repair.md` documents the dry-run-first repair lane, apply-only gate, individual post-fix rescoring, revert-on-worse-score rule, and never-autofix boundaries for payments, auth, unlocks, content protection, copy, visual judgment, creator eligibility, and keyboard runtime behavior.
...
- [x] `src/app/api/creator/subscriptions/route.ts` keeps Fan Pass subscribe paid-source only through `spendCreatorExperienceGumdrops(balance, priceGd, "subscription")`, writes `purchasedOnly: true`, returns typed safe Fan Pass error codes for expected failures, short-circuits duplicate active subscriptions before charging, keeps cancel no-charge, and seeds renewal-readiness fields (`gracePeriodEndsAt`, `renewalFailureCount`, `lastRenewalAttemptAt`, `renewalState`) without executing renewals.
- [x] `src/app/api/creator/bookings/route.ts` maps expected live-time booking failures to typed safe codes (`creator_availability_not_configured`, `slot_outside_availability`, `slot_already_booked`, `insufficient_paid_gumdrops`, and related booking availability states) instead of generic internal server responses while preserving the paid-only transaction path for valid bookings.
...
- [x] `src/app/creators/[username]/CreatorProfileClient.tsx`, `src/lib/problem-state-copy.ts`, and `src/components/Chat/ChatExperience.tsx` pass idempotency keys to server routes without mutating GumDrop balances locally; creator live-time booking and Fan Pass use typed problem-copy helpers so known server problem codes produce precise human messages and paid-GD shortfalls can open Wallet refill.
...
- [x] `scripts/agent/validate-creator-booking-error-copy.ts` and `tests/unit/creator-booking-problem-copy.spec.ts` validate that creator booking expected failures do not use plain thrown `Error` paths or user-facing internal-server fallback copy.
- [x] `scripts/agent/validate-fan-pass-gumdrops-truth.ts` and focused subscription route/copy tests validate Fan Pass paid-source spending, typed expected errors, duplicate-active no double charge, canceled-subscription restart charge, cancel no-charge, and renewal-readiness documentation.
...
- [x] `src/lib/design-system.ts` owns the launch badge containment helper, static-chip affordance helper, and Admin Analytics chart color tokens.
- [x] `src/components/Admin/AdminStatusBadge.tsx` keeps the shared admin status badge on the launch palette with overflow containment and no sky/cyan/slate central status drift.
- [x] `src/components/DropCardParts.tsx` uses shared containment/static-chip helpers for Drop badges and file chips while keeping countdown timers on inherited site font with `aria-live="off"`.
- [x] `src/components/DropPreviewModal.tsx` uses the same badge containment/static-chip helpers for preview file/timer pills.
- [x] `src/components/Admin/AdminAnalyticsCharts.tsx` consumes `KANDYDROPS_CHART_COLORS` for chart accent, grid, axis, legend, and tooltip colors.
- [x] `agent/state/design-system-drift-audit.generated.json`, `docs/agent-truth/design-system-drift.md`, `scripts/agent/validate-design-system-drift.ts`, `package.json`, and `tests/unit/design-system-drift.spec.ts` record and validate the launch design drift gate.
